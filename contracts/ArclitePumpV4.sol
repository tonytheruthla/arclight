...
contract ArclitePumpV4 {
    ...
    constructor(uint256 deploymentFee_, uint256 graduationUsdc_, address treasury_) {
        ...
        ArcliteToken impl = new ArcliteToken();
        impl.initialize("Arclite Token Implementation", "ARK-IMPL", 0, address(this));
        ...
        returns (address token)
    {
        ...
        ArcliteToken(token).initialize(name_, symbol_, TOTAL_SUPPLY, address(this));
        ...
        returns (uint256 tokensOut)
    {
        ...
        ArcliteToken(token).transfer(msg.sender, tokensOut);
    ...
    function migrate(address token) external onlyOwner nonReentrant {
        ...
        ArcliteToken(token).transfer(lpVault, LP_RESERVE);
        _send(lpVault, usdc);
    ...
    function claimCreatorAllocation(address token) external nonReentrant {
        ...
        ArcliteToken(token).transfer(c.creator, CREATOR_ALLOC);
